...
#pragma omp declare reduction(+ : Triple : omp_out = (omp_out + omp_in)) initializer(omp_priv=Triple())
...
Color Scene::trace(Ray const &ray, int depth)
{
    ...
    ****************************************************/
    ...
    #pragma omp parallel for reduction(+:I_D,I_S)
    ...
    if(refraction && material.refractive && kr < 1 && material.ks > 0) {
        ...
        if(refractionScattering && material.frosted) {
            ...
            #pragma omp parallel for reduction(+:rf)
    ...
    if(reflection && material.ks > 0) {
        ...
        if (reflectionScattering) {
            ...
            #pragma omp parallel for reduction(+:rl)
...
void Scene::render(Image &img)
{
    ...
    cml::matrix_rotation_euler(lookMatrix, cml::vector3d(look.x*M_PI/180.0, look.y*M_PI/180.0, look.z*M_PI/180.0), cml::euler_order_xyz);
    
    int rank = mpi::communicator().rank();
    int size = mpi::communicator().size();
    
    std::cout << rank << " " << size << std::endl;
    
    int blockSize = w * h / size;
    
    #pragma omp parallel for
    for(int bIdx = 0; bIdx < blockSize; ++bIdx) {
		int gIdx = rank * blockSize + bIdx;
		int x = gIdx / w;
		int y = gIdx % w;
		
		Color col;
		#pragma omp parallel for collapse(2) schedule(dynamic) reduction(+:col)
		for(int si = 0; si < ss; si++) {
			for(int sj = 0; sj < ss; sj++){
				...
				Ray ray(eye, Vector(dir[0], dir[1], dir[2]).normalized());
				col += trace(ray, 0);
			}
		}
		col /= numSamples;
